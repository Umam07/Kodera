...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (authError) {
        console.warn("Supabase Auth notice, fallback to local login:", authError.message);
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm bg-[#fcfaf5] border-2 border-[#1a3300] rounded-[16px] p-7 sm:p-8 shadow-xs">
        {/* Back Link */}
        <Link
          href="/"
          className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#1a3300]/70 hover:text-[#1a3300] mb-6 transition-colors"
        >
        ...
        <div className="mb-6 text-center">
          ...
          <h1 className="text-2xl font-bold font-bricolage text-[#1a3300] tracking-tight">
            Masuk ke Kodera
          </h1>
          <p className="text-xs text-[#1a3300]/75 mt-1.5 leading-relaxed">
            Simpan progres belajar Java, XP, dan riwayat submission kamu di database Supabase.
          </p>
        </div>
        ...
        <p className="text-[11px] text-center text-[#1a3300]/60 mt-6 leading-relaxed font-mono">
          Autentikasi aman melalui Supabase Auth &amp; Google OAuth 2.0.
        </p>
      </div>
    </div>
  );
